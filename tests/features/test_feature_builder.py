...
from microclimate.features.feature_builder import (
    _bearing_deg,  # noqa: PLC2701  # type: ignore[reportPrivateUsage]
    build_features,
)
...
def _snapshot(
    *,
    ...
    obs = FakeObs(
        frames={
            ref.station_id: make_obs_frame(ref.station_id, ts)
...
def test_identity_columns() -> None:
    ...
    assert df.loc[df["lead_hour"] == 3, "valid_time"].iloc[0] == (
        pd.Timestamp(_T0) + pd.Timedelta(hours=3)
    )
...
def test_valid_hour_encoding_per_lead() -> None:
    ...
    assert row["valid_hour_sin"] == pytest.approx(math.sin(2 * math.pi * 3 / 24.0))  # type: ignore[reportUnknownMemberType]
    assert row["valid_hour_cos"] == pytest.approx(math.cos(2 * math.pi * 3 / 24.0))  # type: ignore[reportUnknownMemberType]


def test_bearing_due_east_is_90() -> None:
    b = _bearing_deg(51.0, -114.0, 51.0, -113.0)
    assert b == pytest.approx(90.0, abs=0.5)  # type: ignore[reportUnknownMemberType]
...
def test_upwind_alignment_matches_formula() -> None:
    ...
    assert df["adv_N1_upwind_align"].to_numpy() == pytest.approx(expected)  # type: ignore[reportUnknownMemberType]
